...
- [x] `T013` 添加站点页面
- [x] `T014` 站点列表页（已展示状态、最近扫描时间、问题总数）
- [x] `T015` 站点详情页骨架（概览/扫描历史/问题/设置占位）
- [~] `T016` 站点状态切换（详情页已支持启停，列表页快捷切换待补）
...
- [~] `T020` 前端状态轮询（详情页已支持手动触发后轮询）
- [ ] `T021` 自动重试
- [x] `T022` 扫描日志查看（详情页已支持查看任务日志与 payload）
...
- [x] URL Guard 基础安全用例（localhost / 私网 IP / 解析到内网）
- [x] 手动触发扫描 service 单测（正常创建 / 并发冲突）
- [~] 登录页错误态展示（已内联表单错误，后续可加更细粒度字段级提示）
